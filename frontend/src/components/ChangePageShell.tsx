...
export function ChangePageShell({ open, onClose, title, eyebrow, subtitle, navigation, footer, children }: {
  open: boolean; onClose: () => void; title: string; eyebrow?: ReactNode;
  subtitle?: ReactNode; navigation?: ReactNode; footer?: ReactNode; children: ReactNode; width?: string
}) {
  if (!open) return null
  return <main className="mx-auto w-full max-w-[88rem] flex-1 space-y-5 px-4 py-6 sm:px-6 lg:px-8">
    ...
    <header className="card space-y-3 p-5 sm:p-6">
      ...
      {navigation ? <div className="border-t border-line pt-4">{navigation}</div> : null}
      {footer ? <div className="border-t border-line pt-4">{footer}</div> : null}
    </header>
    {children}
  </main>
}
